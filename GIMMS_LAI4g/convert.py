...
github_path = (
    "https://github.com/rubisco-sfa/ILAMB-Data/blob/master/GIMMS_LAIg4/convert.py"
)
var = "lai"
...
warnings.filterwarnings(
    "ignore",
    category=Warning,
    message=".*TIFFReadDirectory:Sum of Photometric type-related color channels and ExtraSamples doesn't match SamplesPerPixel.*",
)
    ...
    if not os.path.isfile(local_data):
        ...
        if local_data.endswith(".zip") and not os.path.isfile(zipdir):
            try:
                ...
                print(
                    "There is an issue with the zip file. Try deleting and re-downloading.",
                    e,
                )
                raise
...
def open_grouped_tifs(raster_dict):
    ...
    def coarsen(target_res, data):
        resampled_data = data.coarsen(
            x=(int(target_res / abs(data.rio.resolution()[0]))),
    ...
    for year, months in raster_dict.items():
        ...
        for month, submonth_paths in months.items():
            ...
            for submonth_path in submonth_paths:
                ...
                if (
                    submonth_data.rio.crs is not None
                    and submonth_data.rio.crs.to_epsg() != 4326
                ):
    ...
    for year, months in raster_dict.items():
        ...
        for month, datasets in months.items():
            if len(datasets) > 0:
                ...
                monthly_mean_data["band_1"] = xr.concat(
                    [ds["band_1"] for ds in datasets], dim="datasets"
                ).mean(dim="datasets")
        ...
        for month, dataset in months.items():
            if dataset is not None:
                if concatenated_data:
                    ...
                    if not (
                        dataset["x"].equals(concatenated_data[0]["x"])
                        and dataset["y"].equals(concatenated_data[0]["y"])
                    ):
                        print(
                            f"Rasters for {year}-{month} do not align in x and y dimensions."
                        )
                        return None
...
# 6. create the netcdf
def create_netcdf(
    ds, var, sdate, dstamps, local_path, remote_path, github_path, output_path
):
    # rename dims
    ...
    tb_arr = np.asarray(
        [
            [cf.DatetimeNoLeap(t.dt.year, t.dt.month, t.dt.day) for t in ds["time"]],
            [
                cf.DatetimeNoLeap(
    ...
    v_attrs = {"long_name": long_name, "units": 1}
    ...
    ds["time_bounds"].encoding["units"] = (
        f"days since {sdate.strftime('%Y-%m-%d %H:%M:%S')}"
    )

    # edit global attributes
    generate_stamp = time.strftime(
        "%Y-%m-%d %H:%M:%S", time.localtime(os.path.getmtime(local_path))
    )
    ds.attrs = {
